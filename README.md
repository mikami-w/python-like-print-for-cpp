...
- **Automatic Formatting:**
    ...
    - Prints iterable containers (e.g., `std::vector`, `std::list`, `std::array`) in a `[item1, item2, ...]` format.
    - Prints `std::pair` and `std::tuple` in a `(item1, item2, ...)` format.
...
3. Compile with C++17 support (e.g., `g++ -std=c++17 main.cpp`).
4. Call the function using `pyprint::print(...)`.
